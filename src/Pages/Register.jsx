...
export default function RegisterPage() {
  ...
  const calculateBMI = () => {
    const heightInM =
      formData.height.unit === "cm"
        ? parseFloat(formData.height.value) / 100
        : parseFloat(formData.height.value) * 0.0254;
    const weightInKg =
      formData.weight.unit === "kg"
        ? parseFloat(formData.weight.value)
        : parseFloat(formData.weight.value) * 0.453592;
  ...
  try {
    const userData = {
      ...
      // Keep confirmPassword in the payload
      confirmPassword: formData.confirmPassword
    };

    // Only remove agreeToTerms (not confirmPassword)
  ...
  const handleChange = (e) => {
    ...
    // Special handling for height and weight fields
    if (name === "heightValue") {
      setFormData((prev) => ({
        ...prev,
        height: {
        ...
        <div className="bg-white rounded-xl shadow-xl p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600">
              Join NutriBowl for personalized nutrition
            </p>
          </div>
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between">
              ...
              />
              <div
                className={`w-1/2 h-2 rounded-full ml-2 ${
                ...
                {formData.height && formData.weight && (
                  <div className="p-4 bg-green-50 rounded-lg">
                    ...
                    <p className="text-green-700">
                      BMI: {calculateBMI()} -{" "}
                      {getBMICategory(Number.parseFloat(calculateBMI()))}
                    </p>
                  </div>
                )}
