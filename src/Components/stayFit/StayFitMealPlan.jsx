...
// const apiUrl = 'http://localhost:5001/api/mealplan'; // Replace with your backend URL if different
...
//   // Fetch meal plans from backend on mount
//   useEffect(() => {
//     setLoading(true);
//     axios.get(apiUrl)
//       .then(response => {
//         // Convert array to object keyed by day for easier lookup
//         const plansByDay = {};
//         response.data.forEach(plan => {
//           plansByDay[plan.day] = {
//             'VEG MEAL': plan.vegMeal || { Lunch: [], Dinner: [] },
//             'NON-VEG MEAL': plan.nonVegMeal || { Lunch: [], Dinner: [] }
//           };
//         });
//         setMealData(plansByDay);

//         // Select first day by default if not selected yet
//         const days = Object.keys(plansByDay);
...
//       })
//       .catch(() => {
//         setError('Failed to load meal data from server');
//         setLoading(false);
//       });
...
//   // Calculate nutritional totals for a list of meal items
//   const calculateMealTotals = (meals) => {
//     return meals.reduce((total, item) => ({
//       calories: total.calories + (parseInt(item.Calories) || 0),
//       protein: total.protein + (parseInt(item.Protein) || 0),
//       carbs: total.carbs + (parseInt(item.Carbs) || 0),
//       fat: total.fat + (parseInt(item.Fat) || 0)
//     }), { calories: 0, protein: 0, carbs: 0, fat: 0 });
//   };

//   if (loading) return <div className="text-center mt-20 text-green-700 font-semibold">Loading meal plans...</div>;
//   if (error) return <div className="text-center mt-20 text-red-600 font-semibold">{error}</div>;
//   if (!selectedDay || !mealData[selectedDay]) return <div className="text-center mt-20 font-semibold">No meal plans available.</div>;
...
//             {/* Current Selection Display */}
//             <div className="flex flex-wrap justify-center gap-3 mt-4">
//               <span className="px-4 py-2 bg-green-600 text-white rounded-full font-medium shadow-sm flex items-center" aria-label="Selected day">
//                 <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
//                 </svg>
//                 {selectedDay}
//               </span>
//               <span className="px-4 py-2 bg-green-600 text-white rounded-full font-medium shadow-sm flex items-center" aria-label="Selected meal type">
//                 <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
//                 </svg>
//                 {selectedMealType.replace("MEAL", "").replace("-", " ").trim()}
...
//               { label: "Total Fat", value: lunchTotals.fat + dinnerTotals.fat, unit: "g", icon: "🥑" }
//             ].map((item, index) => (
//               <div key={index} className="bg-green-50 p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow" role="region" aria-label={item.label}>
//                 <div className="flex items-center justify-between">
//                   <h4 className="text-lg font-medium text-green-700">{item.label}</h4>
//                   <span className="text-xl">{item.icon}</span>
//                 </div>
...
//           </div>
//           <div className="overflow-x-auto">
//             <table className="min-w-full divide-y divide-green-200" role="table" aria-label="Meal plan pricing">
//               <thead className="bg-green-50">
//                 <tr>
//                   <th className="px-6 py-4 text-left text-sm font-medium text-green-700 uppercase tracking-wider">Duration</th>
//                   <th className="px-6 py-4 text-center text-sm font-medium text-green-700 uppercase tracking-wider">Vegetarian (1 Meal)</th>
...
//           <div className="p-6 bg-green-50 border-t border-green-200">
//             <button
//               className="w-full py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200 transform hover:scale-[1.01]"
//               aria-label="Get started with your meal plan"
//               onClick={() => alert('Redirect to signup or purchase flow (implement as needed)')}
//             >
//               Get Started with Your Meal Plan
//             </button>
//           </div>
...
const MEAL_API = 'http://localhost:5001/api/mealplan'; // Your existing stay-fit meal plans endpoint
const PRICING_API = 'http://localhost:5001/api/plans/type/stay-fit'; // New Pricing API endpoint
...
const StayFitMealPlan = () => {
  ...
  // Fetch meal plans and pricing on mount
  useEffect(() => {
    ...
    axios.get(MEAL_API)
      ...
      .catch(() => {
        setErrorMeals('Failed to load meal data from server');
        setLoadingMeals(false);
      });

    // Fetch pricing dynamically
    axios.get(PRICING_API)
      .then(response => {
        setPricing(response.data);
        setLoadingPricing(false);
      })
      .catch(() => {
        setErrorPricing('Failed to load pricing data from server');
        setLoadingPricing(false);
      });
  ...
  if (loadingMeals) return <div className="text-center mt-20 text-green-700 font-semibold">Loading meal plans...</div>;
  if (errorMeals) return <div className="text-center mt-20 text-red-600 font-semibold">{errorMeals}</div>;
  if (!selectedDay || !mealData[selectedDay]) return <div className="text-center mt-20 font-semibold">No meal plans available.</div>;
  ...
  // Render pricing table with loading/error states
  const renderPricing = () => {
    if (loadingPricing) {
      return <div className="text-center py-8 text-green-700 font-medium">Loading pricing data...</div>;
    }
    if (errorPricing) {
      return (
    ...
    if (!pricing || pricing.length === 0) {
      return <div className="text-center py-8 text-gray-500 font-medium">No pricing data available.</div>;
    }
