...
//   const apiUrl = 'http://localhost:5001/api/weight-gain-meals'; // Updated for weight gain meals
...
//       // Set default selected day to the first available day if current selection doesn't exist
//       const availableDays = Object.keys(transformedData);
//       if (availableDays.length > 0 && !transformedData[selectedDay]) {
...
//             <svg className="mx-auto h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2M4 13h2m8-8v2m0 0V3m0 4a2 2 0 100 4 2 2 0 000-4z" />
//             </svg>
//           </div>
...
//           <p>No {mealTime.toLowerCase()} items available for this day and meal type.</p>
//         </div>
//       ) : (
//         <div className="overflow-x-auto">
//           <table className="w-full divide-y divide-gray-200">
...
const MEAL_API = 'http://localhost:5001/api/weight-gain-meals'; // For fetching meal plans
const PRICING_API = 'http://localhost:5001/api/plans/type/weight-gain'; // For fetching pricing plans
...
const WeightGainMealPlan = () => {
  ...
  // Fetch meals and pricing on component mount
  useEffect(() => {
    window.scrollTo(0, 0);
    fetchMealPlans();
    fetchPricing();
  ...
  const fetchMealPlans = async () => {
    try {
      ...
      // Transform API meal data structure to your UI format
      const transformedData = {};
      data.forEach(plan => {
        transformedData[plan.day] = {
          "VEG MEAL": plan.vegMeal || { Lunch: [], Dinner: [] },
  ...
  // Calculate meal totals (calories, protein, carbs, fat)
  const calculateMealTotals = meals =>
    meals.reduce(
      (total, item) => ({
        calories: total.calories + (parseInt(item.Calories) || 0),
        ...
        <table className="min-w-full divide-y divide-green-200">
          <thead className="bg-green-50">
            <tr>
              ...
              <th className="px-6 py-4 text-center text-sm font-medium text-green-700 uppercase tracking-wider">
                Vegetarian (1 Meal)
              </th>
              <th className="px-6 py-4 text-center text-sm font-medium text-green-700 uppercase tracking-wider">
                Vegetarian (2 Meals)
          ...
          <tbody className="bg-inherit divide-y divide-green-200">
            {pricing.map((row, idx) => (
              <tr key={row._id || idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-green-50'}>
                <td className="px-6 py-4 whitespace-nowrap font-semibold text-gray-900">{row.duration}</td>
                <td className="px-6 py-4 whitespace-nowrap text-center text-green-600 font-semibold">{row.veg1Meal}</td>
                <td className="px-6 py-4 whitespace-nowrap text-center text-green-600 font-semibold">{row.veg2Meal}</td>
                <td className="px-6 py-4 whitespace-nowrap text-center text-green-600 font-semibold">{row.nonVeg1Meal}</td>
  ...
  // Handle loading or error states for meal data
  if (loadingMeal) {
    return (
      <div
  ...
  // Extract selected meals and totals
  const selectedMealData = mealData[selectedDay]?.[selectedMealType] || { Lunch: [], Dinner: [] };
  const lunchMeals = selectedMealData.Lunch || [];
  const dinnerMeals = selectedMealData.Dinner || [];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-10">
          <div className="bg-inherit bg-opacity-90 backdrop-blur-xl rounded-xl shadow-lg p-8 border border-green-100">
            <h1 className="text-4xl font-bold text-green-800 mb-4">Weight Gain Meal Plans</h1>
            <p className="text-lg text-green-600 mb-8">
              High-calorie nutrition for healthy weight gain and muscle building
            </p>
            ...
            <div className="mb-6">
              ...
              >
                {loadingMeal ? 'Refreshing...' : 'Refresh Data'}
              </button>
            </div>
        ...
        <div className="rounded-xl shadow-lg p-6 mb-10 bg-inherit bg-opacity-90 backdrop-blur-xl border border-green-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={idx} className="bg-inherit p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-green-100">
                <div className="flex items-center justify-between">
                  <h4 className="text-lg font-medium text-green-700">{item.label}</h4>
                  <span className="text-xl">{item.icon}</span>
                </div>
        ...
        <div className="rounded-xl shadow-lg overflow-hidden mb-10 bg-white bg-opacity-90 backdrop-blur-sm border border-green-100">
          <div className="p-6 bg-gradient-to-r from-green-600 to-green-700">
            ...
            <p className="text-green-100 text-center mt-2">
              Choose the plan that fits your weight gain goals
            </p>
          </div>
          {renderPricing()}
          <div className="p-6 bg-green-50 border-t border-green-200">
